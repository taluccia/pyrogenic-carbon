...
```{r}
mod1_graph = ggplot(data = mod1_graph1, aes(x = live.dead, y = response, color = live.dead) ) + 
...
```{r}
mod2_graph = ggplot(data = mod2_graph2, aes(x = live.dead, y = response, color = live.dead) ) + 
   geom_errorbar(width = .2, lwd = .75, aes(ymin = lcl, ymax = ucl), position = position_dodge(width = .75)) + 
geom_point(size = 3, position = position_dodge(width = .75)) + 
  labs(x=(NULL), y= "g BPCA /\nkg material") +
  scale_color_manual(values = c("tan", "darkgreen")) + 
...
```{r}
mod3_graph = ggplot(data = mod3_graph3, aes(x = live.dead, y = response, color = live.dead) ) + 
   geom_errorbar(width = .2, lwd = .75, aes(ymin = lcl, ymax = ucl), position = position_dodge(width = .75)) + 
geom_point(size = 3, position = position_dodge(width = .75)) + 
...
## One Explanatory variable Figure

```{r fig.height=5.25, fig.width=4.75}

exp1_figure1 = cowplot::plot_grid(mod1_graph, mod2_graph, mod3_graph, labels = c("A", "B", "C"), ncol = 1) 
exp1_figure1
```
...
ggsave("../figures/2019-12-19_exp1_dead-live_analysis.jpeg", plot = exp1_figure2, width = 4.75, height = 5.25, units = c("in"), dpi=600 )
```
